...
func InitializeScenario(ctx *godog.ScenarioContext) {
	...
	*/
	...
	ctx.Before(func(ctx context.Context, sc *godog.Scenario) (context.Context, error) {
		...
		// It's ok if this returns an error, it just means the file is empty
		bs, _ := ioutil.ReadFile("globalVariable.json")

		var globalVariable map[string]string

		_ = json.Unmarshal(bs, &globalVariable)

		for i, v := range globalVariable {
			scenario.APIContext.Cache.Save(i, v)
		}
	...
	*/
	...
	ctx.After(func(ctx context.Context, sc *godog.Scenario, err error) (context.Context, error) {
		parse := make(map[string]string, 0)

		for i, v := range scenario.APIContext.Cache.All() {
			parse[i] = fmt.Sprintf("%v", v)
		}

		b, _ := json.Marshal(parse)

		if err := ioutil.WriteFile("globalVariable.json", b, 0666); err != nil {
			checkErr(err)
		}
